...
describe("FOLLOW SYSTEM SERVICE", function() {
    ...
    this.beforeAll(async () => {
        ...
        user1 = await new UserModel({ email: email1, username: username1 }).save()
        user2 = await new UserModel({ email: email2, username: username2 }).save()
    })
    ...
    describe("GetFollowing", function() {
        it("Gets the usernames of those who a given user follows", async () => {
            ...
            //clean up
            user1.followingIDs = user1.followingIDs.filter(followingID => followingID.toString() !== user2.id)
            await user1.save()
        })
    ...
    describe("GetFollowers", function() {
        it("Gets the usernames of those who follow a given user", async () => {
            ...
            //clean up
            user1.followersIDs = user1.followersIDs.filter(followerID => followerID.toString() !== user2.id)
            await user1.save()
        })
